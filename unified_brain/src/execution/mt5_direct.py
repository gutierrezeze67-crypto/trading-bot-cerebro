...
class MT5DirectExecutor:
    ...
    def close_position(self, ticket: int, deviation: int | None = None, volume: float | None = None) -> OrderResult:
        ...
        un volume menor al de la posicion es un cierre PARCIAL -- necesario
        para el TP1 parcial del gestor de posicion del swing (ver
        swing_live.py), portado del mismo metodo ya en produccion para el
        scalping."""
        if not self.is_connected():
            return OrderResult(success=False, error="No conectado a MT5")
    ...
    def get_position_state(self, ticket: int) -> tuple[str, Any | None]:
        ...
        Portado del mismo metodo ya en produccion para el scalping."""
        if not self.is_connected():
            return "UNKNOWN", None
        try:
            positions = mt5.positions_get(ticket=ticket)
